...
var (
	...
	argClientStart             = flag.Int("start-client", 0, "Start client no")
	...
	argGlobalTimeout           = flag.String("global-timeout", "30m", "Timeout spanning all operations")
...
func main() {
	go func() {
		for {
			if msgCount != lastCount {
				log.Printf("Msgcount is %d %d %d ", msgCount, msgCount-lastCount, time.Now().Unix())
	...
	publisherClientIdTemplate = fmt.Sprintf("subscriptions/%s/registries/ReplacewithRegistryId/devices/Stresser", *argSubscriptionName) + "%d"
	topicNameTemplate = "/ReplacewithRegistryId/Stresser%d/events"
	...
	stopStartLoop := false
	startClient := *argClientStart
	for cid := startClient; cid < startClient+*argNumClients && !stopStartLoop; cid++ {
	...
	for finEvents < *argNumClients && !stopWaitLoop {
		select {
		case msg := <-resultChan:
			results[msg.WorkerId-startClient] = msg
	...
	log.Print("Total published ", msgCount, " | ", time.Now().UTC().Unix())
	os.Exit(exitCode)
}
